...
// 페이지네이션 변수
 ...
 int totalFundingCount = fMgr.getTotalFundingCount(); // 전체 펀딩 개수
 int totalPages = (int) Math.ceil((double) totalFundingCount / pageSize); // 총 페이지 수

// 현재 페이지에 해당하는 펀딩 리스트 가져오기
 fundingList = fMgr.getFundingListByPage(currentPage, pageSize);
        ...
        <form action="funding_delete.jsp" method="post">
	        <table>
	          ...
	          <tr>
	            ...
	            <td><a href="./../sub/funding/detail.jsp?num=<%= funding.getNum() %>"><%= funding.getTitle() %></a></td>
	        ...
	        <nav class="page_index_wrap">
				    <a href="?page=<%= currentPage - 1 %>" class="pagePrev" <%= currentPage == 1 ? "style='display:none;'" : "" %>>이전</a>
