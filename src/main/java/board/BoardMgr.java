...
public class BoardMgr {
	...
	private Connection getConnection() {
		try {
			String url = "jdbc:mysql://localhost:3306/happypotato"; // DB URL
	...
	public int getMaxId() {
		...
		try {
			conn = pool.getConnection(); // DBConnectionMgr을 통해 커넥션 가져오기
			...
			pool.freeConnection(conn, pstmt, rs); // 커넥션 해제
		}
	...
	public String getMaxIdTitle() {
		String title = null;
		String sql = "SELECT title FROM noticestbl ORDER BY id DESC LIMIT 1";
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;

		try {
			conn = pool.getConnection(); // 연결 풀에서 커넥션 가져오기
			pstmt = conn.prepareStatement(sql);
			rs = pstmt.executeQuery();
			...
			pool.freeConnection(conn, pstmt, rs); // 커넥션 반환
		}
