...
public class DonationMgr {
    ...
    public void updateDonationStatus(DonationBean donation) {
        Connection conn = null;
        PreparedStatement pstmt = null;

        try {
            conn = pool.getConnection();
            String sql = "UPDATE donaraisingTBL SET status = ? WHERE subnum = ?";
            ...
            // 종료 여부
            boolean isEnded = (diff < 0 || donation.getRaisedAmount() >= donation.getGoalAmount());
            String status = isEnded ? "종료됨" : "진행중";

            pstmt = conn.prepareStatement(sql);
            pstmt.setString(1, status);
            pstmt.setInt(2, donation.getId());

            pstmt.executeUpdate(); // DB업데이트
        } catch (SQLException e) {
            e.printStackTrace();
        } finally {
    ...
    public int raisedAmount(int subnum, String donation_type) {
        ...
        try {
        	...
        	if(rs.next()) {
        		...
        		if(donation_type.equals("donation")) {
    ...
    public int[] getTotalUserAmount() {
    	...
    	try {
    		con = pool.getConnection();
    		sql = "SELECT COUNT(DISTINCT user_id) as totalUser, sum(amount) as totalAmount FROM donationstbl where donation_type = 'donation'";
